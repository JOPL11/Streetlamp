...
const App = () => {
    ...
    const handleButtonClick = () => {
        ...
        //gsap.set('.base', { opacity: 0, duration: 0});
        gsap.to('.base', { opacity: 1, duration: 3, delay:3, ease:'power1.easeInout' });
        setContentVisible(true);
        // Check if audio is already loaded and playing
        if (audioRef.current) {
            audioRef.current.pause(); // Pause the existing audio
...
useEffect(() => {
  if (contentVisible) {
    const tl = gsap.timeline({
      scrollTrigger: {
        trigger: ".base", // The container that will trigger the scroll
        start: "top top", // Start the animation when the top of .base hits the top of the viewport
        end: "bottom bottom", // End the animation when the bottom of .base hits the bottom of the viewport
        scrub: 1, // Smooth scrubbing
        markers: true,  // Enable markers to debug the scroll animation
      },
    });
    ...
    return (
        ...
        <div className='headline'
          style={{
            ...
            zIndex: 1501, // Ensure it's above other elements  <Rain />
          }}
        >
          June 30, 2039 
        </div>
            <Canvas 
                ...
                <Bloom luminanceThreshold={0.1}  mipmapBlur luminanceSmoothing={0.1} intensity={5} />
         ...
         <div className="base" style={{ display: contentVisible ? 'block' : 'none', userSelect: 'none' }}>
          <div className="section1">
                    <p className="title">
                        ...
                        <p className="p">Traffic’s a nightmare at five hundred feet, a ballet of chaos where the dancers are blind and the music’s too fast.  <br /><br /> Everyone’s in a rush to get nowhere, leaving reason in the rearview.  <br /><br />And here he was, wondering if any of this is real, or just another loop in the endless simulation. 
