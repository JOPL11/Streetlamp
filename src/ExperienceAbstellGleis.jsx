...
//import random2D from './shaders/includes/random2D.glsl';  // Loaded vertex shader include   UseShaderInteraction
//import effects from './shaders/includes/effects.glsl'; // Loaded fragment shader include
import UseShaderInteraction from './UseShaderInteraction';
...
*/
  ...
  // Use shader interaction on the hologram
  UseShaderInteraction(unicornRef, camera);
  ...
  cam2.traverse((child) => {
   ...
   const screenImage = useLoader(THREE.TextureLoader, './DeathStar.png');
     ...
     */
     ...
     floor.position.y = 1; // Adjust the position to be just below the objects
      ...
      droneRef.current.position.x = Math.sin(Date.now() * 0.001) * 0.050;
        ...
        gsap.to(droneRef.current.position, { x: 0.5, y: 2.5, z: 11, duration: 1 });
      ...
      */
        ...
        void main()
              {
                ...
                modelPosition.x += (random2D(modelPosition.xz + uTime) - 0.2) * glitchStrength;
                modelPosition.z += (random2D(modelPosition.zx + uTime) - 0.2) * glitchStrength;
      ...
      useEffect(() => {
        const loadModel = async () => {
            ...
            const unicorn = await loader.loadAsync('./unicorn.obj');
        ...
        const updateUniforms = ({ clock }) => {
            const elapsedTime = clock.getElapsedTime();
            hologram.uniforms.uTime.value = elapsedTime;
            hologram2.uniforms.uTime.value = elapsedTime + 2.02;
            hologram3.uniforms.uTime.value = elapsedTime + 4.04;
        };
    
        useFrame(updateUniforms);
    
        // Animate unicorn rotation
        gsap.fromTo(unicornRef.current.rotation, { rotation: -5 }, {
            y: 0.125,
            duration: 15,
            ease: "power1.inOut",
            yoyo: true,
        ...
        // Ensure that unicornRef is fully initialized before passing it to UseShaderInteraction
        // This prevents unicornRef from being undefined when used in UseShaderInteraction
        // UseShaderInteraction should be called within useEffect to ensure proper initialization
        UseShaderInteraction(unicornRef.current, camera);
    }, [camera]);
    ...
    <ambientLight color={0xffffff} intensity={0.002} />
      ...
      {unicornRef.current && (
            <primitive 
                object={unicornRef.current}
                ref={unicornRef}
                scale={[0.0040, 0.0040, 0.0018]}
